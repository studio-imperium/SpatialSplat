...
class FlowEulerCfgSampler:
    ...
    def _inference_model(
        self, model, x_t, t, cond=None, control=None, control_scale=1.0
    ):
        ...
        if control is not None and control.shape[0] == 1 and batch > 1:
            control = control.repeat(batch, 1, 1)
        return model(
            x_t, t_scaled, cond, control=control, control_scale=control_scale
        )

    def _cfg_prediction(
        self, model, x_t, t, cond, neg_cond, guidance_scale,
        ...
        pred_v = self._inference_model(
            model, x_t, t, cond, control=control, control_scale=control_scale
        )
        if isinstance(guidance_scale, dict):
            if not any(s > 1 for s in guidance_scale.values()):
                return pred_v
            neg_pred_v = self._inference_model(
                model, x_t, t, neg_cond, control=control,
