...
def _rotation_matrix_xyz(rotation_degrees: tuple[float, float, float]) -> np.ndarray:
    x, y, z = np.deg2rad(rotation_degrees)
...
def _primitive_world_to_local(
    vectors: np.ndarray, primitive: Primitive
) -> np.ndarray:
    rotation = primitive.rotation_degrees
    if rotation is None or (abs(rotation[0]) < _EPS and abs(rotation[2]) < _EPS):
        return _world_to_local(
            vectors, primitive.yaw_degrees if rotation is None else rotation[1]
        )
    return vectors @ _rotation_matrix_xyz(rotation)
...
def _primitive_local_to_world(
    ...
    return vectors @ _rotation_matrix_xyz(rotation).T
...
def _intersect_sphere(
    ...
    hit = origins + directions * np.where(valid, t, 0.0)[..., None]
...
def _intersect_box(
    ...
    local_origins = _primitive_world_to_local(origins - center, primitive)
    local_directions = _primitive_world_to_local(directions, primitive)
    ...
    hit_local = local_origins + local_directions * np.where(valid, t, 0.0)[..., None]
    ...
    normals = _primitive_local_to_world(normals_local, primitive)
    normals[~valid] = 0
    return t, normals.astype(np.float32)
